...
- Direct PSICHIC scoring for competition compliance
...
def iterative_sampling_loop(
    ...
    3. Score with PSICHIC models directly
    ...
    while True:
        ...
        # STEP 2: Score with PSICHIC models directly
        bt.logging.info(f"[Miner] 🔬 Scoring with PSICHIC models...")
        
        all_target_results = []
        for model in target_models:
            try:
                res = model.score_molecules(filtered_smiles)
                all_target_results.append(res['predicted_binding_affinity'].tolist())
            except Exception as e:
                bt.logging.error(f"[Miner] Target model scoring failed: {e}")
                all_target_results.append([0.0] * len(filtered_smiles))
        
        all_antitarget_results = []
        for model in antitarget_models:
            try:
                res = model.score_molecules(filtered_smiles)
                all_antitarget_results.append(res['predicted_binding_affinity'].tolist())
            except Exception as e:
                bt.logging.error(f"[Miner] Antitarget model scoring failed: {e}")
                all_antitarget_results.append([0.0] * len(filtered_smiles))
