...
def get_smiles_from_reaction_cached(name: str) -> Optional[str]:
    """Cached version of get_smiles_from_reaction for performance"""
    try:
...
def generate_elite_offspring(
    ...
    while len(offspring) < n and attempts < max_attempts:
        ...
        # Decide whether to mutate each component (safer checking)
...
def run_sampler(
    ...
    Returns:
        Dictionary with format: {"molecules": [...], "smiles": [...]}
    """
    global _learner
    ...
    # Format output - MUST INCLUDE BOTH MOLECULES AND SMILES
    sampler_data = {
